...
while cap.isOpened():
	...
	upperPath = "/usr/local/share/OpenCV/haarcascades/haarcascade_upperbody.xml"
        imageGray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
	upperCascade = cv2.CascadeClassifier(upperPath)
	upperRect = upperCascade.detectMultiScale(imageGray, scaleFactor=3.1, minNeighbors=1, minSize=(1,1)) 
